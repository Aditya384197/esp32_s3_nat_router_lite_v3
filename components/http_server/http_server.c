...
static bool pbkdf2_sha256(const char *pw,const uint8_t *salt,size_t slen,uint8_t out[32],uint32_t iters){mbedtls_md_context_t ctx;const mbedtls_md_info_t *md;int rc;mbedtls_md_init(&ctx);md=mbedtls_md_info_from_type(MBEDTLS_MD_SHA256);rc=(md&&mbedtls_md_setup(&ctx,md,1)==0)?mbedtls_pkcs5_pbkdf2_hmac(&ctx,(const unsigned char*)pw,strlen(pw),salt,slen,iters,32,out):-1;mbedtls_md_free(&ctx);return rc==0;}
...
static esp_err_t logout_handler(httpd_req_t *req){if(!origin_ok(req))return ESP_OK;uint8_t c[16];char user[33];if(!require_session(req,true))return ESP_OK;char v[]="__Host-session=; Max-Age=0; HttpOnly; Secure; SameSite=Strict; Path=/";httpd_resp_set_hdr(req,"Set-Cookie",v);portENTER_CRITICAL(&auth_mux);for(int i=0;i<AUTH_SESSION_MAX;i++)if(sessions[i].used&&strcmp(sessions[i].user,user)==0)sessions[i].used=false;portEXIT_CRITICAL(&auth_mux);security_headers(req);httpd_resp_set_type(req,"application/json");return httpd_resp_sendstr(req,"{\"ok\":true}");}
