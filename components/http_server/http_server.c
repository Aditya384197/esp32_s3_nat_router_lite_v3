...
static bool origin_ok(httpd_req_t *req){char h[160];if(httpd_req_get_hdr_value_str(req,"Origin",h,sizeof(h))!=ESP_OK)return true;return strcmp(h,"http://192.168.4.1")==0||strcmp(h,"http://router.local")==0;}
...
{if(!request_via_ap_interface(req))return forbidden_wrong_interface(req);
    ...
    char v[]="session=; Max-Age=0; HttpOnly; SameSite=Strict; Path=/";
...
{
    /* Plain HTTP on the local AP, not HTTPS. A self-signed certificate
     * has no real trust chain (there is no CA to anchor it to this
     * specific device), so it provides little real protection against
     * an active on-path attacker while actively breaking the two
     * things this page most needs to do: auto-open when a phone joins
     * the AP (captive-portal detection uses an embedded mini-browser
     * that typically refuses untrusted certs and simply shows a blank
     * page - this is a well-known OS-level limitation, not something
     * fixable in firmware), and opening cleanly when someone types the
     * IP into a normal browser (which instead shows a "not secure"
     * interstitial). Every real consumer router (TP-Link, Netgear,
     * ASUS, etc.) uses plain HTTP for exactly this reason: the AP's
     * own Wi-Fi password is the actual access boundary, not TLS on an
     * admin page nobody outside that Wi-Fi network can even reach.
     * Login and CSRF protection, rate limiting, and PBKDF2 password
     * hashing are unaffected - all of that logic is independent of
    ...
    c.server_port = port;
    c.task_priority = tskIDLE_PRIORITY + 3;
    c.core_id = 1;
    c.stack_size = 8192;
    ...
    if (httpd_start(&h, &c) != ESP_OK) return NULL;
